...
const KEY_LOOK_SPEED = 1.5 # radians per second
...
func _physics_process(delta):
	...
	# Keyboard camera controls (works in 3D only)
	if Input.mouse_mode == Input.MOUSE_MODE_CAPTURED and !mode_2d:
		var yaw := 0.0
		var pitch := 0.0
		...
		rotate_y(yaw * KEY_LOOK_SPEED * delta)
		head.rotate_x(pitch * KEY_LOOK_SPEED * delta)
